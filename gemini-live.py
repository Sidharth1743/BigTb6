...
def parse_args():
    ...
    parser.add_argument(
        "--system-instruction", type=str, help="System instruction text"
    )
    parser.add_argument(
    ...
    if args.screen_capture_fps:
        try:
            ...
            print(
                "Screen capture requires the mss library. Install with 'pip install mss'"
            )
            quit
                ...
                if evt.get("setupComplete", None) is not None:
                    ...
                    if parts := sc.get("modelTurn", {}).get("parts"):
                        if text := parts[0].get("text"):
                            ...
                            if mime_type == "audio/pcm" and sample_rate == str(
                                SPEAKER_SAMPLE_RATE
                            ):
                                ...
                                print(
                                    f"Unsupported mime type or sample rate: {mime_str}"
                                )
    ...
    async def run(self):
        ...
        if AUDIO_INPUT:
            self.mic_audio_in = self.p.open(
                ...
                frames_per_buffer=int(MIC_SAMPLE_RATE / 1000)
                * 2
                * 50,  # 50ms (S16_LE is 2 bytes)
                start=False,
            )
        ...
        try:
            self.ws = await websockets.connect(
                uri=f"wss://{HOST}/ws/google.ai.generativelanguage.{API_VERSION}.GenerativeService.BidiGenerateContent?key={os.getenv('GOOGLE_API_KEY')}"
            )
